...
function DisplayFirestoreComponent({data}) {
    ...
    const nextPage = (currentIndex) => {
        while(currentIndex<data.length-10){
            return setCurrentIndex(currentIndex + 10);
        }
    }
    ...
    (
        <div>
            {data.slice(currentIndex, currentIndex+10).map((pokemon) => 
    ...
    (
        <nav className='d-flex justify-content-center'>
            <ul className='pagination'>
                ...
                <li className='page-item'>
                    <p className='page-link bg-danger text-warning' id='pagination'>Page {Math.ceil(currentIndex/10)+1} of {Math.ceil(data.length/10)}</p>
                </li>
                <li className='page-item'>
    ...
    return (
        <div className='container my-2 bg-warning text-danger'>
...
export default connect(mapStateToProps, {retrievePokemons, findPokemonByName})(DisplayFirestoreComponent)